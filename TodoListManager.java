...
public class TodoListManager {
                    ...
                    if(getConfirm("마감기한을")){
                        ...
                        if(getConfirm("체크 가능 시작시점을")) {
                            ...
                            if(!canCreateNormalJob(deadlineDate,
                                    deadlineTime,startDate,startTime,-1)){
                    ...
                    if(aim.equals("c")){
                        ...
                        while(true){
                            ...
                            if(!todo.isHasDeadline()) {
                                ...
                                result = getDeadlineDate("마감일을",dateNow);
                                if(result.getFormatDate() == null){
                                    System.out.println("이전 단계로 돌아갑니다.");
                                    continue outerLoop2;
                                }
                            ...
                            if(todo.isCanCheckAfterCheckStartDate()){
                                LocalDateTime start =
                                        LocalDateTime.of(todo.getCheckStartDate().getFormatDate(),todo.getCheckStartTime());
                                LocalDateTime end =
                                ...
                                if(todo.getBusy() == BusyType.BUSY_Y){
                                    if(!canCreateBusyJob(deadline,deadTime,
                                            todo.getCheckStartDate().getFormatDate(),
                                            todo.getCheckStartTime(),idx)){
                        ...
                        while(true){
                            ...
                            if(!todo.isCanCheckAfterCheckStartDate()){
                                ...
                                dateResult = getStartDate("시작날짜",null);
                                if(dateResult.getFormatDate() == null){
                                    System.out.println("이전 단계로 돌아갑니다.");
                                    continue outerLoop2;
                                }
                            ...
                            if(todo.isHasDeadline()){
                                LocalDateTime end =
                                        LocalDateTime.of(todo.getDeadline().getFormatDate(),
                                                todo.getDeadTime());
                                LocalDateTime start =
                                ...
                                if(todo.getBusy() == BusyType.BUSY_Y){
                                    if(!canCreateBusyJob(todo.getDeadline().getFormatDate(),
                                            todo.getDeadTime(),
                                            startDate,
                                            startTime,idx)){
                                        System.out.println("일정이 겹쳐 수정할 수 " +
                                    ...
                                    if(!canCreateNormalJob(todo.getDeadline().getFormatDate(),
                                            todo.getDeadTime(),
                                            startDate,
                                            startTime,idx)){
                                        System.out.println("일정이 겹쳐 수정할 수 없습니다.");
                            ...
                            todo.setCheckStartDate(dateResult);
                        ...
                        while(true){
                            if(todo.getBusy() == BusyType.BUSY_N){
                                ...
                                if(!todo.isHasDeadline()){
                                    ...
                                    result = todo.getDeadline();
                                    endDate = todo.getDeadline().getFormatDate();
                                    endTime = todo.getDeadTime();
                                }
                                // 시작시간 확인
                                if(!todo.isCanCheckAfterCheckStartDate()){
                                    System.out.println("바쁨할일은 시작일이 필수입니다.");
                                    dateResult = getStartDate("시작날짜",endDate);
                                    if(dateResult.getFormatDate() == null){
                                        System.out.println("이전 단계로 돌아갑니다.");
                                        continue outerLoop2;
                                    }
                                    ...
                                    dateResult = todo.getCheckStartDate();
                                    startDate = todo.getCheckStartDate().getFormatDate();
    ...
    public void updateRegularList(List<RegularList> regulerList) {
        System.out.println("======== 반복 할 일 수정하기 ========");
        String input;
        outerLoop1: while(true){
                    ...
                    if(aim.equals("c")){
                        ...
                        outerLoop3:while(true){
                            ...
                            if(!first.isHasDeadline()) {
                                ...
                                System.out.println("설정된 마감일이 존재합니다.(Y,N)\n수정" +
                                               " :" +
                                        "Y\n삭제 : N");
                                if(!getConfirmYN()){
                                    // 시작일이 설정되어있지않으면 삭제 불가
                                    if(!first.isCanCheckAfterCheckStartDate()){
                                        System.out.println("반복 할일은 시작일 또는 " +
                                                "마감일중 하나는 필수입니다.");
                                        continue outerLoop2;
                                    }
                                    LocalDate startDate = null;
                                    ...
                                    if(first.isCanCheckAfterCheckStartDate()){
                                        startDate =
                                                first.getCheckStartDate().getFormatDate();
                                        startTime = first.getCheckStartTime();
                                        rawStart =
                                                first.getCheckStartDate().getRawDate();
                                    ...
                                    System.out.println("해당 반복할일의 마감일이 삭제" +
                                            " " +
                                            "되었습니다.");
                                    showRegularList(regulerList);
                                    return;
                            ...
                            String rawStart = null;
                            if(first.isCanCheckAfterCheckStartDate()){
                                startDate = first.getCheckStartDate().getFormatDate();
                                startTime = first.getCheckStartTime();
                                rawStart =
                                        first.getCheckStartDate().getRawDate();
                            ...
                            if(regularList.getCycleType() == CycleType.WEEKLY){
                                updateList = createRegularListWeekly(first,
                                        start,end,regularList.getCycleEnd(),idx);
                            }else{
                                updateList = createRegularListMonthly(first,
                                        start,
                                        rawStart,
                        ...
                        outerLoop3:while(true){
                            ...
                            if(!first.isCanCheckAfterCheckStartDate()) {
                                ...
                                System.out.println("설정된 시작날짜가 존재합니다.(Y,N)\n수정" +
                                        " :" +
                                        "Y\n삭제 : N");
                                if(!first.isHasDeadline()){
                                    System.out.println("반복 할일은 시작일 또는 " +
                                ...
                                if(!getConfirmYN()){
                                    ...
                                    String rawEnd = null;
                                    if(first.isHasDeadline()){
                                        deadline = first.getDeadline().getFormatDate();
                                        rawEnd =
                                                first.getDeadline().getRawDate();
                                        deadTime = first.getDeadTime();
                                        end = LocalDateTime.of(deadline,
                                                deadTime);
                                    }
                                    List<TodoList> updateList;
                                    if(regularList.getCycleType() == CycleType.WEEKLY){
                                    ...
                                    regularList.setTodoList(updateList);
                                    System.out.println("해당 반복할일의 시작일이 삭제" +
                                            " " +
                                            "되었습니다.");
                                    showRegularList(regulerList);
                                    return;
                            ...
                            String rawEnd = null;
                            if(first.isHasDeadline()){
                                endDate = first.getDeadline().getFormatDate();
                                rawEnd = first.getDeadline().getRawDate();
                        ...
                        while(true){
                            ...
                            if(todo.getBusy() == BusyType.BUSY_N){
                                ...
                                if(!todo.isHasDeadline()){
                                    System.out.println("바쁨할일은 마감일이 필수입니다.");
                                    System.out.println("마감일을 우선 설정해주세요.");
                                    continue outerLoop2;
                                }
                                // 시작시간 확인
                                if(!todo.isCanCheckAfterCheckStartDate()){
                                    System.out.println("바쁨할일은 시작일이 필수입니다.");
                                    System.out.println("시작일을 우선 설정해주세요.");
                                    continue outerLoop2;
                                }
                                start =
                                        LocalDateTime.of(todo.getCheckStartDate().getFormatDate(), todo.getCheckStartTime());
                                end =
                                        LocalDateTime.of(todo.getDeadline().getFormatDate(), todo.getDeadTime());
                                List<TodoList> newList = null;
                                todo.setBusy(BusyType.BUSY_Y);
                                if(regularList.getCycleType() == CycleType.WEEKLY){
                                    newList = createRegularListWeekly(todo,
                                            start,end,
                                            regularList.getCycleEnd(),idx);
                                }else{
                                    newList = createRegularListMonthly(todo,
                                            start,todo.getDeadline().getRawDate(),
                                            end,todo.getCheckStartDate().getRawDate(),
                                            regularList.getCycleEnd(),idx);
                                }
                                if(newList == null){
                                    todo.setBusy(BusyType.BUSY_N);
                                ...
                                regularList.setTodoList(newList);
                                System.out.println((idx + 1) + "번 목록을 안바쁨할일로 " +
                                        "변경하였습니다.");
                                showRegularList(regulerList);
                                return;
                            }
                                ...
                                if(start != null && end != null){
                                    ...
                                    if(first.getBusy() == BusyType.BUSY_Y){
                                        if(checkWeekGap(start,end)){
                                            System.out.println("시작일과 마감일의 범위가 " +
                                                    "일주일을 초과하여, 주단위 반복으로 변경 불가합니다" +
                                                    ".");
                                            System.out.println("시작일 또는 마감일을 " +
                                                    "수정해주세요.");
                                            continue outerLoop2;
                                        }
                                    }
                        ...
                        while(true){
                            ...
                            if(todo.isCanCheckAfterCheckStartDate()){
                                start =
                                        LocalDateTime.of(todo.getCheckStartDate().getFormatDate(),
                                         todo.getCheckStartTime());
                                firstDate = start.toLocalDate();
                                firstTime = start.toLocalTime();
                                rawStart =
                                        todo.getCheckStartDate().getRawDate();
                            }
                            if(todo.isHasDeadline()){
                                end = LocalDateTime.of(todo.getDeadline().getFormatDate(),
                                        todo.getDeadTime());
                                firstDate = end.toLocalDate();
                                firstTime = end.toLocalTime();
                                rawEnd =
                                        todo.getDeadline().getRawDate();
                            }

                            // 반복마감시간 입력받기
                            DateResult result = getDeadlineDate("반복 종료일을",
                                    dateNow);
                            if(result.getFormatDate() == null){
                                System.out.println("이전 단계로 돌아갑니다.");
                                continue outerLoop2;
                            ...
                            System.out.println("해당 목록의 반복 종료일을 " + cycleEnd +
                                    "로 변경하였습니다.");
                            showRegularList(regulerList);
                            return;
                        }
    ...
    public void checkList(List<TodoList> todoList) {
        ...
        while (true) {
            ...
            if (input.equals("c")) {
                ...
                // 체크 시작일/시간 검사
                if (todo.isCanCheckAfterCheckStartDate()) {
                    LocalDate todoDate = todo.getCheckStartDate().getFormatDate();
                ...
                if (todo.isHasDeadline() && !todo.isCanCheckAfterDeadline()) {
                    LocalDate todoDate = todo.getDeadline().getFormatDate();
    ...
    private List<TodoList> createRegularListWeekly(TodoList list,
        ...
        LocalDateTime befoStart = null;
        LocalDateTime befoEnd = null;
        while(standard.isBefore(cycleEnd)){
            if(end != null && start != null){
                if(list.getBusy() == BusyType.BUSY_Y){
                    // 이전 회차와 겹치는지 검증
                    if(befoStart != null && befoEnd != null){
                        if(befoStart.isAfter(start) && befoStart.isBefore(end)){
                            System.out.println("이전 회차와 겹처 생성할 수 없습니다.");
                            return null;
                        }
                        if(befoEnd.isAfter(start) && befoEnd.isBefore(end)){
                            System.out.println("이전 회차와 겹처 생성할 수 없습니다.");
                            return null;
                        }
                    }
                    if(!checkCreateRegularBusyJob(start,end,idx)){
                        System.out.println("일정이 겹처 " +
                                "생성할 수 없습니다.");
                        return null;
                    }
                }else if(list.getBusy() == BusyType.BUSY_N){
                    if(!checkCreateRegularJob(start,end,idx)){
                        System.out.println("일정이 겹처 " +
                                "생성할 수 없습니다.");
                        return null;
                    }
            ...
            TodoList newTodo =
                    TodoList.createRegularList(
                            list.getTitle(), hasD,
                            new DateResult(endDate.toString().replace("-",""),endDate),
                            endTime,
                            list.getBusy(), hasS,
                            new DateResult(startDate.toString().replace("-",
                                    ""),startDate),
                            startTime,
    ...
    private List<TodoList> createRegularListMonthly(TodoList list,
                                             ...
                                             LocalDateTime endDay,
                                             String rawEnd,
                                             LocalDateTime cycleEnd, int idx
    ) {
        // 시작일 또는 마감일이 null 일때는?
        // 바쁨일때 리스트의 이전 날짜의 마지막이 만들려는 시작일보다 이후이면 불가능
        // 날짜 정보 조건 추가해야함
        ...
        while(standard.isBefore(cycleEnd)){
            ...
            if(end != null && start != null){
                if(list.getBusy() == BusyType.BUSY_Y){
                    ...
                    if(befoStart != null && befoEnd != null){
                        if(befoStart.isAfter(start) && befoStart.isBefore(end)){
                            System.out.println("이전 회차와 겹쳐 생성할 수 없습니다.");
                            return null;
                        }
                        if(befoEnd.isAfter(start) && befoEnd.isBefore(end)){
                            System.out.println("이전 회차와 겹쳐 생성할 수 없습니다.");
            ...
            TodoList newTodo =
                    TodoList.createRegularList(
                            list.getTitle(), hasD,
                            new DateResult(rawEnd,endDate),
                            endTime,
                            list.getBusy(), hasS,
                            new DateResult(rawStart,startDate),
                            startTime,
                            list.isCanCheckAfterDeadline()
                    );
            result.add(newTodo);
    ...
    private boolean checkCreateRegularBusyJob(LocalDateTime currStart,
        ...
        for(TodoList list : todoList){
            if(list.getBusy().equals(BusyType.BUSY_Y)){
                LocalDateTime ed = LocalDateTime.of(list.getDeadline().getFormatDate(),
                        list.getDeadTime());
                LocalDateTime st =
                        LocalDateTime.of(list.getCheckStartDate().getFormatDate(),
        ...
        for(RegularList lists : regulerList){
            if(idx >= 0 && regulerList.get(idx).equals(lists))
                continue;
            for(TodoList list : lists.getTodoList()){
                if(list.getBusy().equals(BusyType.BUSY_Y)){
                    ...
                    if(list.isHasDeadline() && list.isCanCheckAfterCheckStartDate()){
                        LocalDateTime ed =
                                LocalDateTime.of(list.getDeadline().getFormatDate(),
                                list.getDeadTime());
                        LocalDateTime st =
                                LocalDateTime.of(list.getCheckStartDate().getFormatDate(),
                                        list.getCheckStartTime());
    ...
    private boolean checkCreateRegularJob(LocalDateTime currStart,
        ...
        for(TodoList list : todoList){
            if(list.getBusy().equals(BusyType.BUSY_Y)){
                LocalDateTime st = LocalDateTime.of(list.getCheckStartDate().getFormatDate(),
                        list.getCheckStartTime());
                LocalDateTime ed = LocalDateTime.of(list.getDeadline().getFormatDate(),
                        list.getDeadTime());
                System.out.println("기존 시작 날짜:" + st);
    ...
    public void showRegularListDetail(List<TodoList> list){
        ...
        for(TodoList todo : list){
            ...
            // 마감일이 미설정시
            if(!todo.isHasDeadline()){
                dateTime = LocalDateTime.of(todo.getCheckStartDate().getFormatDate(),
                        todo.getCheckStartTime());
            ...
            // 마감일이 있음
                ...
                dateTime = LocalDateTime.of(todo.getDeadline().getFormatDate(),
                        todo.getDeadTime());
                //마감일 이후 체크 가능
    ...
    private boolean canCreateBusyJob(LocalDate endDate,LocalTime endTime,
        ...
        for(TodoList list : todoList){
            // 같은 할일은 제외
            if(idx >= 0 && todoList.get(idx).equals(list))
                continue;
            // 바쁨 할일과 비교
            if (list.getBusy().equals(BusyType.BUSY_Y)) {
                LocalDateTime todoStart =
                        LocalDateTime.of(list.getCheckStartDate().getFormatDate()
                        ,list.getCheckStartTime());
                LocalDateTime todoEnd = LocalDateTime.of(list.getDeadline().getFormatDate(),
                        list.getDeadTime());
